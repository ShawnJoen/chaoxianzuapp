...
public class AsyncImageLoader {
	...
	public Bitmap loadImage(ImageView imageView, String imageUrl) {
		...
		if (bitmap != null) {
			Log.d("leslie", "image exists in memory");
			return bitmap;
		}
		...
		if (bitmap != null) {
			Log.d("leslie", "image exists in file");
			// 重新缓存到内存中
	...
	class ImageDownloadTask extends AsyncTask<String, Integer, Bitmap> {
		...
		protected Bitmap doInBackground(String... params) {
			...
			Log.d(BasicInfo.NETWORK_IMG_TAG , "doInBackground 1 ::: ");
			try {
				imageUrl = params[0];
				...
				Log.d(BasicInfo.NETWORK_IMG_TAG, "doInBackground 2 ::: ");

				Bitmap bitmap = getBitmapFromDisk(imageUrl);
				if (bitmap != null) {

					Log.d(BasicInfo.NETWORK_IMG_TAG , "doInBackground 3 ::: ");
		...
		protected void onPostExecute(Bitmap result) {
			...
			Log.d(BasicInfo.NETWORK_IMG_TAG , "onPostExecute 1 ::: ");
			if (result != null) {

				Log.d(BasicInfo.NETWORK_IMG_TAG , "onPostExecute 2 ::: ");
				// 通过 tag 来防止图片错位
				//if (imageView.getTag() != null && imageView.getTag().equals(imageUrl)) {

					Log.d(BasicInfo.NETWORK_IMG_TAG , "onPostExecute 3 ::: ");
